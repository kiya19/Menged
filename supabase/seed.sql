...
--   reception@menged.et  /  Reception@123   (role: reception — can also
--                                            edit/delete Issue Material)
--   reports@menged.et    /  Reports@123     (role: reports — restricted
--                                            to Reports, Tasks, Finance)
insert into users (name, email, password_hash, role) values
  ...
  ('Betelhem A.', 'reception@menged.et', '$2b$10$.JWOtRPrw9xrI/zRe981muE1W5ZMS/KRqP9AXmAr/in1hlkuz8LaG', 'reception'),
  ('Yared G.',  'reports@menged.et',   '$2b$10$adyY0KoEZcQpNCocuanrdu2ROpJeOjgv7/Buv9AJahNjkpnT096Ea', 'reports')
...
-- Note: the "cashier" column here stores the name shown in the Issue
-- Material UI, which is now labelled "IT Technician" there (equipment is
-- issued to the IT technician who services the POS hardware). The column
-- name is kept as-is to avoid a schema rename; only the UI label changed.
insert into material_issues (cashier, material, quantity, issue_date, notes) values
  ('Dawit K.',  'Charger', 1, '2026-08-03', 'POS charging unit'),
  ('Abel M.',   'Cable',   1, '2026-08-04', 'Replacement USB cable');
