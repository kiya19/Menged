...
the `Authorization: Bearer <token>` header. Four roles:
...
- **staff** — day-to-day work: view everything, assign fleet, add finance
  ...
  management (gets a `403`). Can add Issue Material records but not edit
  or delete them.
- **reception** — same day-to-day access as staff, plus can edit and
  delete Issue Material records (the only difference from staff).
- **reports** — restricted account intended for reporting/sync use. The
  frontend only shows this role the Reports, Tasks, and Finance Audit
  views (everything else redirects back to Reports). This is a frontend
  navigation restriction, not a backend data restriction — the API
  endpoints themselves don't further filter by this role.
...
| reception | `reception@menged.et` | `Reception@123` |
| reports | `reports@menged.et` | `Reports@123` |
...
| Method | Path | Body | Who |
|---|---|---|---|
| GET | `/` | – (newest first) | any logged-in user |
| POST | `/` | `{cashier, plateNo, material, quantity, date, notes, serialNo}` | any logged-in user |
| PUT | `/:id` | any subset of the POST body | admin, reception |
| DELETE | `/:id` | – | admin, reception |

The `cashier` field is what the Issue Material UI labels "IT Technician" —
the field/column name wasn't renamed to avoid a schema/API break.
